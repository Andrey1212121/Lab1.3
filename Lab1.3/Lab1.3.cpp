...
public:
    ...
    void attackEnemy(Entity& target) override {
        int damage = attack - target.getDefence();
        if (damage > 0) {
            // Шанс на огненную атаку (30%)
            if (rand() % 100 < 30) {
                damage += 5; // Дополнительный урон от огня
...
public:
    ...
    void CritStrike(Entity& target) {
        int damage = attack - target.getDefence();
        if (damage > 0) {
            // Шанс на крит атаку (20%)
            if (rand() % 100 < 20) {
                damage *= 5; // Дополнительный урон
                std::cout << "Critical damage!! ";
            }
            target.takeDamage(damage);
            std::cout << name << " attacks by Critical Strike " << target.getName() << " for " << damage << " damage!\n";
        }
        else {
...
int main() {
    ...
    // Создание объектов
    Character hero("Hero", 500, 20, 10);
    Monster goblin("Goblin", 50, 15, 5);
    Monster dragon("Dragon", 150, 25, 20);
    Boss boss("Master of blade", 200, 50, 40);
